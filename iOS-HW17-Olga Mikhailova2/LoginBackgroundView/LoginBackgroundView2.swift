...
class LoginBackgroundView2: UIView {
    // Настройки
    ...
    private func setupLayers() {
        ...
        whiteLayer.path = self.createWavePath()
        ...
        fadeLayer.frame = CGRect(x: 0,
                                 y: bounds.height * (1 - self.whiteAreaHeightRatio) - self
                                     .waveHeight,
                                 width: bounds.width,
                                 height: self.waveHeight * 2)
        layer.addSublayer(fadeLayer)
    }

    private func createWavePath() -> CGPath {
        let path = UIBezierPath()
        let startY = bounds.height * (1 - self.whiteAreaHeightRatio)
        ...
        // Волна
        path.addCurve(to: CGPoint(x: bounds.width, y: startY),
                      controlPoint1: CGPoint(x: bounds.width * 0.3, y: startY - self.waveHeight),
                      controlPoint2: CGPoint(x: bounds.width * 0.7, y: startY + self.waveHeight))
